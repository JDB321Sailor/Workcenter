...
/* The Workcenter panes. Each is a deep-linkable route rendering the single view. */
export const VIEW_META = {
  workspace: { supportsSection: false },
  files: { supportsSection: false },
  chat: { supportsSection: false },
  mail: { supportsSection: false },
};
...
/* Extract the pane segment from a path, defaulting to the workspace */
export const viewFromPath = (path) => {
  const seg = (path || '').split('/').filter(Boolean)[0];
  return VIEW_META[seg] ? seg : 'workspace';
};
