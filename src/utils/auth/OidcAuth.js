...
/* True when Workcenter's own login page should be shown, instead of jumping to the IdP */
...
class OidcAuth {
  ...
  async login() {
    ...
    if ($store.state.rootConfig?._bootstrap?.authenticated === false) {
      ...
      if (getApiAuthState().ok) {
        throw new Error(
          'OIDC login succeeded, but Workcenter still received unauthenticated config. '
  ...
  /* Mirror the OIDC user into the localStorage keys other parts of the shell read */
