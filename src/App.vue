<template>
  <div id="workcenter" :style="topLevelStyleModifications" :class="subPageClassName">
...
import CriticalError from '@/components/PageStructure/CriticalError.vue';
...
export default {
  ...
  computed: {
    ...
    topLevelStyleModifications() {
      ...
      let styles = '';
      // No footer in the shell, so the reserved height depends only on the page title.
      if (!vc.pageTitle) styles += '--footer-height: 1rem;';
      else styles += '--footer-height: 5rem;';
    ...
    applyCustomStyles() {
      ...
      if (this.appConfig.externalStyleSheet) {
        ...
        urls.forEach((url) => {
          ...
          link.setAttribute('data-external-source', 'workcenter');
