/**
 * Workcenter's custom zero-dependency v-tooltip directive for Vue 3
...
function injectStyles() {
  ...
  style.id = 'workcenter-tooltip-styles';
  style.textContent = `
.workcenter-tooltip {
  position: absolute;
  top: 0; left: 0;
...
.workcenter-tooltip.is-visible {
  visibility: visible;
  opacity: 1;
}
.workcenter-tooltip .tooltip-inner {
...
.workcenter-tooltip .tooltip-arrow {
  position: absolute;
  width: 0; height: 0;
  border-style: solid;
...
.workcenter-tooltip[data-placement^="top"] .tooltip-arrow {
  bottom: -${ARROW_SIZE}px;
  border-bottom-width: 0;
  border-top-color: var(--description-tooltip-color, #7efff5);
}
.workcenter-tooltip[data-placement^="bottom"] .tooltip-arrow {
  top: -${ARROW_SIZE}px;
  border-top-width: 0;
  border-bottom-color: var(--description-tooltip-color, #7efff5);
}
.workcenter-tooltip[data-placement^="left"] .tooltip-arrow {
  right: -${ARROW_SIZE}px;
  border-right-width: 0;
  border-left-color: var(--description-tooltip-color, #7efff5);
}
.workcenter-tooltip[data-placement^="right"] .tooltip-arrow {
  left: -${ARROW_SIZE}px;
  border-left-width: 0;
...
.workcenter-tooltip.in-modal-tt { z-index: 11; }
@media (prefers-reduced-motion: reduce) {
  .workcenter-tooltip { transition: none; }
}
  `;
...
function show(el) {
  ...
  state.showTimer = setTimeout(() => {
    ...
    tip.className = 'workcenter-tooltip tooltip';
    tip.id = state.id;
...
function ensureGlobalClick() {
  ...
  document.addEventListener('click', () => {
    document.querySelectorAll('.workcenter-tooltip.is-visible').forEach((tip) => {
...
export default {
  mounted(el, binding) {
    ...
    const id = `workcenter-tt-${idCounter++}`;
