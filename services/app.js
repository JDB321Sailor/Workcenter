...
const app = express()
  ...
  // Workcenter renders one view, so any GET that is not a static asset renders the
  // shell. Other methods and requests that look like a missing asset fall through
  // to a 404, so a broken reference surfaces instead of silently returning HTML.
  .use((req, res, next) => {
    if (req.method !== 'GET' && req.method !== 'HEAD') return next();
    const isAssetRequest = /\.[a-z0-9]{2,5}$/i.test(req.path);
    if (isAssetRequest) return next();
    return res.status(200).sendFile('index.html', { root: path.join(rootDir, 'dist') }, (err) => {
      if (err) safeEnd(res, errBody('Not Found'), 404);
    });
  });
