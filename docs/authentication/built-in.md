...
Workcenter ships with a simple username + password auth system baked in, as well as the option to enforce this server-side with standard HTTP auth.
...
Restart Workcenter for changes to take effect. You'll be bounced to a login screen. Enter your username and the plaintext password that hashes to the value above (`12345` if you copy-pasted the example. Please change it).
...
- `password` - Alternative to `hash`. The name of a `VITE_APP_*` environment variable holding the plaintext password. Workcenter hashes the env var's value at runtime. This requires building the app from source (don't work with pre-built Docker image)
...
Then run Workcenter with `VITE_APP_ALICIA_PASSWORD=supersecret`. Workcenter hashes the value at login time and compares it to what the user types. The env var name must start with `VITE_APP_`; anything else is rejected.

> [!WARNING]
> Caveat: env-var passwords only resolve in the browser if the variable was set when the Workcenter bundle was *built*. The official Docker image is pre-built, so it can't pick up env-var users you set at runtime for the frontend. For env-var users to work, either build Workcenter yourself or use `hash` inline instead. The server-side enforcement does read env vars at runtime regardless.
...
With just `auth.users` set in `conf.yml`, and no env vars, Workcenter shows a login screen, and after a successful login it sets a session cookie containing a SHA-256 token. The SPA refuses to render the dashboard without that cookie.
...
Same `auth.users` list, but the server enforces it too. Set this env var when starting Workcenter:
...
### Static credentials (no Workcenter login page)

If you don't want the Workcenter login page at all and would rather have the browser handle auth via a native HTTP basic-auth dialog, skip `auth.users` and set:
...
Problem: Workcenter logs `Invalid user object. Must have user and either a hash or password param`.<br>
...
Problem: Login works when hitting Workcenter directly, but breaks behind nginx/Caddy/etc.<br>
Solution: Reverse proxies sometimes strip the `Authorization` header or mishandle `Set-Cookie`. Make sure your proxy is configured to pass `Authorization` through, and that `Set-Cookie` from Workcenter reaches the browser unmodified. For nginx, `proxy_pass_request_headers on;` is the default, but custom `proxy_set_header` blocks can override it.
...
- **No password reset, no lockout, no rate limiting.** A determined attacker can hammer the login endpoint. Front Workcenter with a proxy that does rate limiting if you care
...
sequenceDiagram
    ...
    actor User
    participant Browser
    participant Workcenter

    User->>Browser: Open Workcenter URL
    Browser->>Workcenter: GET /
    Workcenter-->>Browser: SPA bundle (HTML/JS/CSS, always public)
    Browser->>Workcenter: GET /conf.yml
    Workcenter-->>Browser: conf.yml (with auth.users defined)
...
```mermaid
flowchart LR
    Req([Request reaches Workcenter]) --> Mw{Has Authorization<br/>Bearer header?}
...
sequenceDiagram
    ...
    Note over Browser,Workcenter: SPA assets (HTML/JS/CSS) are always public

    User->>Browser: Open Workcenter URL
    Browser->>Workcenter: GET /
    Workcenter-->>Browser: SPA bundle

    Note over Browser,Workcenter: First fetch of conf.yml is now gated
    Browser->>Workcenter: GET /conf.yml
    Workcenter-->>Browser: 401 WWW-Authenticate: Basic
    ...
    Browser->>Workcenter: GET /conf.yml<br/>Authorization Bearer header set from cookie
    Workcenter->>Workcenter: customAuthorizer checks token matches any user
    Workcenter-->>Browser: 200 + conf.yml

    Note over Browser,Workcenter: From now on, every API + .yml fetch carries the bearer
    Browser->>Workcenter: GET /system-info<br/>Authorization Bearer header set from cookie
    Workcenter-->>Browser: 200 + system info
```
